...
namespace LIB.API.Persistence.Repositories
{
        ...
        public class RefundRepository : IRefundRepository
    {
      ...
      .ToListAsync();
            ...
            string dAccountBranch ="00129";
            string DAccountNo = "00110611283";
            string DAccountHolderName = "00312365168";
            bool isTransferSuccess = await CreateTransferAsync(transaction,refundRequest.Amount, DAccountNo, dAccountBranch, transaction.DAccountNo, CAccountBranch, refundRequest.RefundReferenceCode, refundRequest);
        ...
        //    bool refundConfirmed = await ConfirmRefundAsync(refundConfirmationRequest);
            //if (!refundConfirmed)
            //{
            //    var transferRecordErrored = new Refund
            //    {
            ...
            //        CBSResponseStatus =  "Didnt reach cbs", // Set response status
            //        CBSErrorReason = "refund confirmaation failed",  // Store error message if the transfer fails
            //        CBSRequestTimestamp = DateTime.UtcNow,  // Timestamp when request was made
            //        CBSResponseTimestamp = DateTime.UtcNow ,  // Response timestamp if successful
            //        CBSIsSuccessful =false,
            //    };
            //    await _context.refunds.AddAsync(transferRecordErrored);
            ...
            //    //throw new Exception("Refund confirmation API failed");
            //}
           ...
           // var soapResponse = await _soapClient.SendSoapRequestAsync("https://10.1.7.85:8095/createTransfer", xmlRequest, soapAction);
            ...
            if (isSuccess)
            {
                var refundConfirmationRequest = new RefundConfirmationRequest
                {
                    ...
                    Amount = refundRequest.Amount.ToString(),
                    Currency = refundRequest.Currency,
                    RefundAccountNumber = transaction.DAccountNo,
                    RefundDate = DateTime.UtcNow, // Use current date if needed
                    RefundFOP = refundRequest.RefundFOP,
                    Status = "1", // Success
                    Remark = "Refund successfully processed",
                    AccountHolderName = transaction.DAccountName,
                };
        ...
        {
            string apiUrl = "https://flyprod.ethiopianairlines.com/Lion/api/V1.0/Lion/ConfirmRefund";
            ...
            try
            {
                using (var httpClient = new HttpClient { Timeout = TimeSpan.FromSeconds(20) }) // Add timeout here
                {
                    ...
                    {
                        await LogErrorToAirlinesErrorAsync("API Error", refundRequest.RefundAccountNumber, "Failed",
                            responseString, "ConfirmRefund", refundRequest.RefundReferenceCode);
                    }

                    RefundConfirmationResponse? jsonResponse = null;
                    try
                    {
                        jsonResponse = JsonSerializer.Deserialize<RefundConfirmationResponse>(responseString);
                    }
                    catch
                    {
                        await LogErrorToAirlinesErrorAsync("Deserialization Error", refundRequest.RefundAccountNumber,
                            "Failed", "Invalid JSON response", "ConfirmRefund", refundRequest.RefundReferenceCode);
                    }
                    ...
                    var confirmRefund = new ConfirmRefund
                    {
                        ...
                        Status = jsonResponse?.Status != null ? "1" : "0",
                        ...
                        ResponseMessage = jsonResponse?.message ?? "No response or invalid response",
            ...
            {
                await LogErrorToAirlinesErrorAsync("Exception Occurred", refundRequest.RefundAccountNumber,
                    "Failed", ex.Message, "ConfirmRefund", refundRequest.RefundReferenceCode);
